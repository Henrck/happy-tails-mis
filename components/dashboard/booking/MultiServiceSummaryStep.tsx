...
// Boarding displays the requested kennel SIZE only; the specific kennel
// number is assigned later by staff in Appointment Check In.
"use client";
...
export default function MultiServiceSummaryStep({
  ...
  const grandTotal = legs.reduce(
    (sum, leg) =>
      sum +
      leg.petSelections.reduce((legSum, sel) => legSum + sel.lineAmount, 0),
    0
  );
    ...
    return (
      <div className="mx-auto w-full max-w-md py-8 text-center sm:py-16">
        ...
        <p className="mt-1 text-sm text-zinc-500">
          {legs.length > 1
            ? `All ${legs.length} services have been booked.`
            : "Your appointment has been booked."}
        </p>
        <button
          type="button"
          onClick={() => router.push("/account")}
          className="mt-6 min-h-12 w-full rounded-full bg-brand-pink px-5 py-3 text-sm font-semibold text-white hover:bg-brand-pink-dark"
        >
          Back to My Pets
        </button>
      </div>
  ...
  return (
    <div className="mx-auto w-full max-w-3xl">
      <div className="text-center">
        <h2 className="text-2xl font-extrabold tracking-tight text-brand-pink sm:text-[28px]">
          Booking Summary
        </h2>
      ...
      <div className="mt-4 space-y-3 sm:mt-5 sm:space-y-4">
        {legs.map((leg) => {
          ...
          const legTotal = leg.petSelections.reduce(
          ...
          return (
            <section
              key={leg.id}
              className="w-full rounded-2xl border-2 border-pink-100 p-4 sm:p-5"
            >
              <div className="flex items-start justify-between gap-3">
                <h3 className="min-w-0 text-base font-bold leading-5 text-brand-pink sm:text-lg">
                  {serviceLabels[leg.serviceChoice]}
                </h3>
                <button
                  type="button"
                  onClick={() => onEditLeg(leg.id)}
                  className="shrink-0 text-xs font-semibold text-brand-pink hover:underline sm:text-sm"
                >
                  Edit
                </button>
              </div>
              ...
              {leg.petSelections.map((sel) => {
                ...
                return (
                  <div
                    key={sel.pet.id}
                    className="mt-3 border-t border-dashed border-pink-100 pt-3"
                  >
              ...
              <div className="mt-3 border-t border-dashed border-pink-100 pt-3 text-sm text-zinc-600">
                ...
                {leg.specialRequests && (
                  <p className="mt-1 break-words">
                    Notes: {leg.specialRequests}
                  </p>
                )}
              </div>

              <p className="mt-2 text-right text-sm font-bold text-brand-pink sm:text-base">
                Subtotal: ₱{legTotal.toLocaleString()}
              </p>
            </section>
          );
        })}
      ...
      <div className="mt-4 flex items-center justify-between gap-3 rounded-2xl bg-brand-tint px-4 py-3.5 sm:mt-5 sm:px-5 sm:py-4">
        <span className="text-sm font-bold text-zinc-800 sm:text-base">
          Grand Total
        </span>
        <span className="text-lg font-bold text-brand-pink sm:text-xl">
          ₱{grandTotal.toLocaleString()}
        </span>
      </div>

      {error && (
        <p className="mt-3 break-words rounded-lg bg-red-50 px-3 py-2 text-sm text-red-600 sm:mt-4">
          {error}
        </p>
      )}
      ...
      <div className="mt-4 grid grid-cols-2 gap-2.5 sm:mt-6 sm:gap-3">
        <button
          type="button"
          onClick={onBack}
          className="min-h-11 rounded-full border-2 border-zinc-300 px-3 py-2.5 text-sm font-semibold text-zinc-500 hover:border-zinc-400 sm:min-h-12 sm:px-6"
        >
          Back
        </button>
        <button
          ...
          disabled={saving}
          className="min-h-11 rounded-full bg-brand-pink px-3 py-2.5 text-sm font-semibold text-white hover:bg-brand-pink-dark disabled:opacity-50 sm:min-h-12 sm:px-6"
        >
          {saving
            ? `Saving ${savedCount}/${legs.length}…`
