...
export default function ProductsPageContent({ products }: { products: Product[] }) {
  ...
  return (
    <>
      ...
      <div className="site-container mt-8 rounded-3xl bg-brand-pink p-5 sm:p-6 md:p-8">
        {visible.length === 0 ? (
          <p className="py-10 text-center text-white">No products in this category yet.</p>
        ) : (
          <div className="grid grid-cols-2 items-stretch gap-4 sm:grid-cols-3 md:grid-cols-5 md:gap-5">
            {visible.map((product) => (
