...
import type { SessionStage } from "@/lib/data/grooming-sessions-mock";

export type SessionFilterState = {
  search: string;
  status: SessionStage | "all";
  groomer: string | "all";
};
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      ...
      <div className="flex-1 min-w-[180px] relative">
        ...
        <input
          type="text"
          placeholder="Search by pet, owner, or groomer"
      ...
      <div className="relative">
        ...
        {panelOpen && (
          <div className="absolute left-0 top-full mt-2 w-64 bg-white rounded-2xl shadow-xl border border-pink-100 p-5 z-20">
            ...
            <div className="mt-4">
              <label className="text-xs font-semibold text-zinc-500">GROOMER</label>
              <select
                value={filters.groomer}
                onChange={(e) => onChange({ ...filters, groomer: e.target.value })}
                className="mt-1 w-full rounded-full border-2 border-brand-pink text-zinc-700 text-sm px-3 py-2 focus:outline-none"
              >
                <option value="all">All Groomers</option>
                {groomerNames.map((name) => (
