"use client";
// Real version — Customer has no age/sex field (that was Owner-specific
// mock data, not a real account attribute), so those rows are dropped
// rather than faked. customer_id (not customer.id truncated) shown as
// the real identifier.
...
export default function OwnerDetailModal({
  ...
  const ownerPets = pets.filter((p) => p.customer_id === customer.id);
      ...
      <div className="w-full max-w-sm bg-white rounded-3xl overflow-hidden max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-6 py-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="text-lg font-bold text-zinc-900">{customer.full_name}</h4>
              <p className="text-xs text-zinc-400">{customer.customer_id}</p>
            </div>
          </div>

          <div className="mt-4 space-y-2 text-sm">
            <Row label="Contact Number" value={customer.phone_number} />
          ...
          <div className="mt-2 space-y-2">
            {ownerPets.length === 0 ? (
              <p className="text-sm text-zinc-400 text-center py-4">No pets on file yet.</p>
            ) : (
              ownerPets.map((pet) => (
                <button
                  key={pet.id}
