...
export default function InventoryPage() {
  ...
  const [stocksSortDirection, setStocksSortDirection] =
    useState<SortDirection>(null);
  ...
  const filteredProducts = useMemo(() => {
    ...
    const enriched = matched.map((p) => {
      ...
      return {
        ...p,
        totalStock,
        statusLabel: totalStock <= p.min_stock ? "Low Stock" : "In Stock",
      };
    });
     ...
     * LOW STOCK PRIORITY
     *
     * Low-stock products are always grouped before normal-stock products.
     * If the user clicks a column header, that column is sorted INSIDE
     * each priority group. This means a low-stock item can never fall
     * below an in-stock item.
     */
    const prioritySorted = [...enriched].sort((a, b) => {
    ...
    if (!productsSortKey || !productsSortDirection) {
      return prioritySorted;
    }

    /*
     * Preserve the low-stock-first rule even when a table header is sorted.
     * sortRows alone would allow an alphabetical/stock sort to move normal
     * products above low-stock products, so sorting is applied per group.
     */
    const lowStock = prioritySorted.filter((p) => p.statusLabel === "Low Stock");
    const normalStock = prioritySorted.filter((p) => p.statusLabel !== "Low Stock");

    return [
      ...sortRows(lowStock, productsSortKey, productsSortDirection),
      ...sortRows(normalStock, productsSortKey, productsSortDirection),
    ];
  }, [
    visibleProducts,
    search,
  ...
  const filteredBatches = useMemo(() => {
    ...
    const matches = batches.filter((b) => {
      if (!q) return true;
      const name =
        products.find((p) => p.id === b.product_id)?.name.toLowerCase() ?? "";
    ...
    const enriched = matches.map((b) => ({
      ...b,
      productName:
        products.find((p) => p.id === b.product_id)?.name ?? b.product_id,
  ...
  const pagedProducts = filteredProducts.slice(
    ...
    productsPage * PAGE_SIZE + PAGE_SIZE
  );

  const pagedBatches = filteredBatches.slice(
    stocksPage * PAGE_SIZE,
    stocksPage * PAGE_SIZE + PAGE_SIZE
  );
  ...
  return (
    <div>
      ...
      {error && (
        <p className="mt-4 text-sm text-red-600 bg-red-50 rounded-lg px-4 py-2.5">
          {error}
        </p>
      )}

      <div className="mt-6">
        <InventoryStats products={products} batches={batches} />
      </div>
      ...
      {tab === "products" && (
        <div className="mt-3 flex gap-2">
          ...
          <button
            onClick={() => handleStatusViewChange("discontinued")}
            className={`text-xs font-semibold px-4 py-1.5 rounded-full transition-colors ${
              statusView === "discontinued"
                ? "bg-zinc-800 text-white"
                : "bg-zinc-100 text-zinc-500 hover:bg-zinc-200"
            }`}
          >
