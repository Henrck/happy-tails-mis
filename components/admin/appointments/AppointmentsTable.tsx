      ...
      <table className="w-full min-w-[820px] text-sm table-fixed">
        ...
        <thead>
          <tr className="bg-brand-pink text-white">
            {COLUMNS.map((c) => (
              <th key={c.label} className="text-left font-semibold px-4 py-3 truncate">
                {c.label}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {rows.length === 0 ? (
            ...
            rows.map((a, i) => (
              <tr
                key={a.id}
                className={`border-t border-pink-50 hover:bg-brand-tint/60 transition-colors ${
                  i % 2 === 1 ? "bg-zinc-50/50" : ""
                } ${isToday(a.scheduled_date) ? "border-l-4 border-l-brand-pink" : ""}`}
              >
                <td
                  className="px-4 py-3.5 font-medium text-zinc-800 truncate"
                  title={a.pets.map((p) => p.name).join(", ")}
                >
                  {a.pets.map((p) => p.name).join(", ") || "—"}
                  {a.pets.length > 1 && (
                    <span className="ml-1.5 text-xs text-zinc-400">({a.pets.length})</span>
                  )}
                </td>

                <td className="px-4 py-3.5 text-zinc-600 truncate" title={a.owner_name}>
                  {a.owner_name}
                </td>
                ...
                <td className="px-4 py-3.5 text-zinc-600 truncate">
                  ...
                  {isToday(a.scheduled_date) && (
                    <span className="ml-1.5 text-[10px] font-bold text-brand-pink">TODAY</span>
                  )}
                </td>

                <td className="px-4 py-3.5 text-zinc-800 font-semibold truncate">
                  ₱{a.total_amount.toLocaleString()}
                </td>

                <td className="px-4 py-3.5">
                  <span
                    className={`inline-block max-w-full truncate text-xs font-semibold px-3 py-1 rounded-full ${
                ...
                <td className="px-4 py-3.5">
                  <button
                    onClick={() => onView(a)}
                    aria-label="View"
                    className="w-8 h-8 rounded-full border-2 border-brand-pink text-brand-pink flex items-center justify-center hover:bg-brand-pink hover:text-white transition-colors"
                  >
