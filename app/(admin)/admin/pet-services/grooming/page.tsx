...
export default function GroomingManagementPage(){
 ...
 async function handleComplete(appointmentPetId:string,petName:string){
  ...
  const {error}=await completeAppointmentPet(appointmentPetId,"groomer");
 ...
 {completedName&&<SessionCompletedModal petName={completedName} onClose={()=>setCompletedName(null)}/>}
 </div>
}